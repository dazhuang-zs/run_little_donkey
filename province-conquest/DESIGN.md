...
- **六维可视化对比**：对决面板展示双方省份名（彩色）、对决名人（前3位）、6维度对比条、比分及胜方标签
- **名人卡片展示**：按省份分组展示参战名人（姓名、称号、6维属性标签，属性颜色区分），卡片采用响应式网格布局
...
2. **展示对决详情** — `updateBattlesList()` 渲染右侧对决列表（对决名人 + 6维对比条 + 比分）；`updateHeroesList()` 更新名人分组卡片
