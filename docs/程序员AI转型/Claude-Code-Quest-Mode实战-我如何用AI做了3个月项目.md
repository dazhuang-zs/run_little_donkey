...
**把决策和执行分离**。/plan 阶段专注决策（做什么、怎么做、有什么风险），Implement 阶段专注执行。决策阶段不应该有执行权限，执行阶段不应该偏离决策。

**用 CLAUDE.md 建立长期记忆**。把项目规范写到 CLAUDE.md，Claude 每次新会话都能继承上下文。

**用清晰的验收标准减少反馈回路**。给 Claude 测试用例或预期输出，它就能自我检查。没有明确标准，你就成了唯一的反馈回路。
...
**第一阶段做的需求理解，到第四阶段已经记不全了。** 每次新阶段开始，Claude 都要重新加载上下文。它会在编码阶段偏离最初的需求理解，写一些最初计划里根本没有的功能。

根本原因：**没有 L1 信息边界层的设计。**

### 局限二：编码和测试是同一个 Agent，自己测自己

Claude 写完代码，自己运行测试，自己判断通过没有。GAN 架构早就证明：**Agent 很难真正否定自己的产出。**

### 局限三：工具权限没有按阶段区分

规划阶段 Claude 可能已经开始写代码，测试阶段还在改需求。没有阶段锁定机制，流水线随时可能崩溃。

### 局限四：失败后没有恢复机制

测试挂了，Claude 在一堆未提交的变更里来回修改，越改越乱。没有状态管理，不知道上次失败的具体原因，通常只能"算了重来"。

### 局限五：报告质量完全依赖最后一次对话

没有结构化模板，内容随机，容易漏项。没有变更清单，没有验收记录。

---

## 三、Qoder 的多专家架构带来了什么

2025年8月，阿里发布了 Qoder，一个 Agentic 编程平台。2026年3月，Qoder 上线了 Experts Mode（专家团模式）。
...
这就是 Quest Mode 借鉴的核心思想。

---

## 四、Quest Mode：结合 /plan 和多专家架构的高阶方法论

Quest Mode 是对 Claude Code /plan 模式的高阶增强方法论，结合了 Qoder 的多专家架构设计思想。它的目标是在 /plan 模式的"先规划后执行"基础上，增加多专家流水线、状态管理、失败恢复。
...
Quest Mode 的 Plan Expert 不是另起炉灶，而是把 /plan 模式的最佳实践（只读约束、风险识别、结构化输出）固化为基础能力。
...
**Review Expert 没有 exec 的写权限**，不能修改代码。修复必须通过 Master Agent 协调 Code Expert 完成。
